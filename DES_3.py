
import class_des
        
def get_DES():

    des = class_des.DES()
    
    des.name = "3"
    des.have_refined_state = 0
    des.s_act = ['0', '1', '2', '3', '4', '5']

    des.event_act =['01', '03', '12', '20', '34', '40', '45',  'finish']

    des.trans_act={
            des.s_act[0]:[[des.s_act[1],des.event_act[0]],
                          [des.s_act[3],des.event_act[1]]],
            des.s_act[1]:[[des.s_act[2],des.event_act[2]]],
            des.s_act[2]:[[des.s_act[0],des.event_act[3]]],
            
            des.s_act[3]:[[des.s_act[4],des.event_act[4]]],
            des.s_act[4]:[[des.s_act[0],des.event_act[5]],
                          [des.s_act[5],des.event_act[6]]],
            des.s_act[5]:[[des.s_act[5],des.event_act[-1]]]
            }


    des.istate_act = des.s_act[0]
    des.fin_state_act = des.s_act[-1]
    des.ap_act = ["3_1", "3_2", "3_3", "3_4", "3_5", "3_6"] 
    des.AP_R = ["3_1", "3_2"]
    des.label_act = {   
                des.s_act[0]:[des.ap_act[2]],
                des.s_act[1]:[des.ap_act[0], des.ap_act[3]],
                des.s_act[2]:[des.ap_act[1]],
                
                des.s_act[3]:[des.ap_act[0], des.ap_act[4]],
                des.s_act[4]:[des.ap_act[1]],
                des.s_act[5]:[des.ap_act[-1]]
        }
    
    des.timed_event = {    
                des.event_act[0]:[0,-1],
                des.event_act[1]:[1,2],
                des.event_act[2]:[2, 3],
                des.event_act[3]:[2,-1],
                des.event_act[4]:[2,3],
                des.event_act[5]:[2,-1],
                des.event_act[6]:[2, -1],
                des.event_act[7]:[0,0]
                    }
    """
    上位TDESとの単位時間のずれ
    0以上1以下の数になる
    """
    des.time_ratio = 0.8
    return des